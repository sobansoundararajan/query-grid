...
public class GroupCompare implements Comparator<QueriedResult> {
    ...
    public int compare(QueriedResult queriedResult1, QueriedResult queriedResult2) {
        for (SortOnGroups functionSortCondition : this.sortOnGroupsList) {
            ...
            double val1 = (double) queriedResult1.getFunctionMap().computeIfAbsent(functionCondition, (k) -> queriedResult1.evaluateFormula(grid, range, functionCondition)).getValue();
            double val2 = (double) queriedResult2.getFunctionMap().computeIfAbsent(functionCondition, (k) -> queriedResult2.evaluateFormula(grid, range, functionCondition)).getValue();
            if (val1 != val2) {
